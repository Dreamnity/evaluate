...
if (isMainThread) {
  ...
  (async () => {
    let chwd = process.chdir;
    process = module = global = {};
    process.exit = () => console.error('no');
    Object.setPrototypeOf(JSON, { require });
    require = hideCall(
      (
        pkg //hideCall protect function from .toString()
      ) => {
        let req = Object.getPrototypeOf(JSON).require; //secret
        let cont = { req, pkg,chwd },
          vm = req('vm');
        vm.createContext(cont);
        vm.runInContext(
          "let local=!req.resolve(pkg).includes('node_modules')",
          cont
        );
    ...
    let oldaaaaaalog = console.log,
      conout = '';
    console = new Proxy(console, {
      get(t, p) {
        return (...e) => {
          conout +=
            '[' + p + '] ' + e.map((e) => convertStr(e)).join(' ') + '\n';
        };
      }
    });
    const result =
